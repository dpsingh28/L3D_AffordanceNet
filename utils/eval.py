...
def evaluation(logger, cfg, model, test_loader, affordance):
    ...
    with torch.no_grad():
        ...
        for i,  temp_data in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
            ...
            _,afford_clip_emb = model(data)
            afford_clip_emb = torch.tanh(afford_clip_emb)
            afford_pred = torch.sigmoid(cos(afford_clip_emb,torch.tanh(affordance_embeddings)))
            ...
            imageio.mimsave('/home/daman/L3D_AffordanceNet_jay/image/pn2/{}_{}_pred.gif'.format(modelcat[0], modelid[0]), pred_point_image.cpu().numpy(), fps=30)
            imageio.mimsave('/home/daman/L3D_AffordanceNet_jay/image/pn2/{}_{}_label.gif'.format(modelcat[0], modelid[0]), label_point_image.cpu().numpy(), fps=30)
