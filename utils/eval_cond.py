...
def evaluation(logger, cfg, model, test_loader, affordance):
    ...
    with torch.no_grad():
        ...
        for i,  temp_data in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
            ...
            for label_idx in range(len(affordances_list)):
                print(label_idx)
                ...
                label_colors = label_colors*(label[:,:,label_idx].squeeze(0).unsqueeze(-1).repeat(1,1,3)[0,0:,:])#*10
                
                afford_colors = afford_colors*(afford_pred.squeeze(0)[:,label_idx].unsqueeze(-1).repeat(1,1,3)[0,0:,:])#*100            
                ...
                imageio.mimsave('/home/daman/L3D_AffordanceNet_jay/image/pn2/{}_{}_affordance_{}_pred.gif'.format(modelcat[0], modelid[0],label_idx), pred_point_image.cpu().numpy(), fps=30)
                imageio.mimsave('/home/daman/L3D_AffordanceNet_jay/image/pn2/{}_{}_affordance_{}_label.gif'.format(modelcat[0], modelid[0],label_idx), label_point_image.cpu().numpy(), fps=30)

    #             L2distance = torch.sum(
    #                 torch.pow(label[:,:,label_idx]-afford_pred, 2), dim=(0, 1))
    #             total_L2distance += L2distance
    #             score = afford_pred.squeeze()
    #             target_score = label[:,:,label_idx].squeeze()
    #             results[i, :, :] = score
